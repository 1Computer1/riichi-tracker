...
import { type Hand, isDora, type TileCode, TilesBySuit } from "../../lib/hand";
...
export function SuitRow({
  ...
  return (
    <div className="flex min-w-min flex-row flex-wrap items-center justify-center gap-1 lg:gap-2">
      <div className="flex min-w-min flex-row flex-wrap items-center justify-center gap-1 lg:gap-2">
        {(sanma && suit === "m"
          ? [TilesBySuit[suit][0], TilesBySuit[suit][8]]
          : TilesBySuit[suit].slice(0, 5)
        ).map((t) => (
          <TileButton
            key={t}
            tile={t}
...
function isDisabled(
  ...
  if (action?.t === "dora" || action?.t === "uradora") {
    ...
    if (
      tile[0] === "0" &&
      allTiles.concat(hand.dora, hand.uradora).some((t) => t === tile)
    ) {
      return true;
    }
  ...
  if (tile[1] === "z") {
    if (action) {
      ...
      return (
        action.t === "chii" || (action.t === "pon" ? count >= 2 : count >= 1)
      );
    }
  } else if (action) {
    if (action.t === "chii") {
      ...
      const adjancents =
        action.tiles.length === 1
          ? [
              (Number(action.tiles[0][0]) || 5) - 1,
              (Number(action.tiles[0][0]) || 5) + 1,
            ]
          : [
              (Number(action.tiles[0][0]) || 5) - 1,
              (Number(action.tiles[1][0]) || 5) + 1,
            ];
...
function countDoraIndicators(tile: TileCode, hand: Hand): number {
  return (
    hand.dora.filter((d) => sameTile(d, tile)).length +
    hand.uradora.filter((d) => sameTile(d, tile)).length
  );
}
